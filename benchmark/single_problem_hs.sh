...
# Fixed settings
COMBO_MODE="knee0"          # knee_point combination, niterations_integration=0
...
TOTAL=$(( N_PTS * N_TRAJ * N_NOISE * N_NITER ))
...
echo "  Combo mode        : $COMBO_MODE  (knee_point, niterations_integration=0)"
    ...
    for n_traj in "${NUM_TRAJ_LIST[@]}"; do
        for noise in "${NOISE_LEVELS[@]}"; do
            for niter in "${NITER_DERIV_LIST[@]}"; do
                ...
                job_name="sp_${PROBLEM}_noise${noise_tag}_pts${n_pts}_traj${n_traj}_niter${niter}"
